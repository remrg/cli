...
describe('replacePlaceholder', () => {
	...
	it('replaces placeholder in a dot-file', () => {
		tmpDir = mkdtempSync(join(tmpdir(), 'remrg-test-'));
		writeFileSync(join(tmpDir, '.env.example'), 'NAME={{ remrg:var project-name }}');

		replacePlaceholder(makeRunner(tmpDir), 'project-name', 'my-app');

		expect(readFileSync(join(tmpDir, '.env.example'), 'utf8')).toBe('NAME=my-app');
	});
	...
	it('replaces placeholder in a file inside a dot-directory', () => {
		...
		mkdirSync(dotDir);
		writeFileSync(join(dotDir, 'settings.json'), '{ "name": "{{ remrg:var project-name }}" }');

		replacePlaceholder(makeRunner(tmpDir), 'project-name', 'my-app');

		expect(readFileSync(join(dotDir, 'settings.json'), 'utf8')).toBe('{ "name": "my-app" }');
	});
